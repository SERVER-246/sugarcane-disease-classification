...
# DEBUG CONFIGURATION
# =============================================================================
DEBUG_MODE = os.environ.get('DBT_DEBUG_MODE', 'true').lower() == 'true'
DEBUG_BACKBONE = os.environ.get('DBT_DEBUG_BACKBONE', 'CustomCoAtNet')
...
EXPORT_CONFIG = {
    ...
    """'openvino': {
        'data_type': 'FP32',            # FP32, FP16, or INT8
        'input_shape': None,            # Override input shape (optional)
        'model_name': None,             # Override model name (optional)
        'compress_to_fp16': False,      # Compress weights to FP16
    },
    'coreml': {
        'minimum_deployment_target': 'iOS13',  # iOS13, iOS14, iOS15, etc.
        'convert_to': 'neuralnetwork',  # 'neuralnetwork' or 'mlprogram'
        'compute_precision': 'FLOAT32', # FLOAT32, FLOAT16
...
# Optional dependency check results (populated at runtime)
EXPORT_DEPENDENCIES = {}
...
def check_export_dependencies():
    ...
    try:
        import torch_model_archiver  # type: ignore
...
def instantiate_arch_stub(arch_name: str, num_classes: int = None) -> nn.Module:
...
def prepare_sample_input(model: nn.Module, input_size: int = 224, batch_size: int = 1, device: str = 'cpu') -> torch.Tensor:
...
def export_model_onnx(model: nn.Module,
    ...
    if not EXPORT_DEPENDENCIES.get('onnx', False):
        result['status'] = 'skipped'
...
def export_model_torchserve_mar(model: nn.Module,
    ...
    if not EXPORT_DEPENDENCIES.get('torchserve', False):
        result['status'] = 'skipped'
...
def export_model_tensorrt_engine(onnx_path: Path,
    ...
    if not EXPORT_DEPENDENCIES.get('tensorrt', False):
        result['status'] = 'skipped'
...
"""
def export_model_openvino_ir(onnx_path: Path,
                             output_dir: Path,
                             arch_name: str,
                             force: bool = False,
                             logger_inst = None) -> dict:
    ...
    xml_path = output_dir / f"{arch_name}_openvino.xml"
    bin_path = output_dir / f"{arch_name}_openvino.bin"
    ...
    if not EXPORT_DEPENDENCIES.get('openvino', False):
        result['status'] = 'skipped'
        result['reason'] = 'OpenVINO not installed. Install: pip install openvino-dev'
        logger_inst.warning(f"  xx OpenVINO export skipped: {result['reason']}")
        return result
    
    if not onnx_path.exists():
        result['status'] = 'failed'
        result['reason'] = 'ONNX file not found (required for OpenVINO conversion)'
        logger_inst.warning(f"  xx OpenVINO export failed: {result['reason']}")
        return result
    
    try:
        if xml_path.exists() and bin_path.exists() and not force:
            result['status'] = 'skipped'
            result['reason'] = 'Files exist'
            result['sha256'] = compute_file_sha256(xml_path)
            return result
        ...
        result['status'] = 'skipped'
        result['reason'] = 'OpenVINO conversion requires mo CLI tool (TODO: implement)'
        logger_inst.info(f"  **** OpenVINO: {result['reason']}")
        
    except Exception as e:
        result['reason'] = f'OpenVINO export failed: {str(e)}'
        logger_inst.error(f"  x OpenVINO export failed: {e}")
    
    return result


def export_model_coreml(onnx_path: Path,
                        output_path: Path,
                        arch_name: str,
                        force: bool = False,
                        logger_inst = None) -> dict:
    ...
    if not EXPORT_DEPENDENCIES.get('coremltools', False):
        result['status'] = 'skipped'
        result['reason'] = 'coremltools not installed. Install: pip install coremltools'
        logger_inst.warning(f"  xx CoreML export skipped: {result['reason']}")
        return result
    
    if not onnx_path.exists():
        result['status'] = 'failed'
        result['reason'] = 'ONNX file not found (required for CoreML conversion)'
        logger_inst.warning(f"  xx CoreML export failed: {result['reason']}")
        return result
    ...
    try:
        ...
        result['sha256'] = compute_file_sha256(output_path)
        result['status'] = 'ok'
        result['reason'] = 'Successfully exported to CoreML'
        
        logger_inst.info(f"  Cool Exported CoreML: {output_path.name}")
        
    except Exception as e:
        result['reason'] = f'CoreML export failed: {str(e)}'
        logger_inst.error(f"  x CoreML export failed: {e}")
...
def export_model_tflite(onnx_path: Path,
    ...
    if not EXPORT_DEPENDENCIES.get('tensorflow', False):
        result['status'] = 'skipped'
...
def smoke_check_onnx(onnx_path: Path,
    ...
    if not EXPORT_DEPENDENCIES.get('onnxruntime', False):
        result['status'] = 'skipped'
...
"""
def smoke_check_openvino(xml_path: Path,
                         sample_input: torch.Tensor = None,
                         logger_inst = None) -> dict:
    ...
    if not EXPORT_DEPENDENCIES.get('openvino', False):
        result['status'] = 'skipped'
        result['reason'] = 'OpenVINO runtime not available for smoke check'
        logger_inst.warning(f"    xx OpenVINO smoke check skipped: {result['reason']}")
        return result
    ...
    try:
        ...
        # Run inference
        output = compiled_model([sample_input.numpy()])[0]
...
def smoke_check_coreml(mlmodel_path: Path,
                       sample_input: torch.Tensor = None,
                       logger_inst = None) -> dict:
    ...
    if not EXPORT_DEPENDENCIES.get('coremltools', False):
        result['status'] = 'skipped'
        result['reason'] = 'coremltools not available for smoke check'
        logger_inst.warning(f"    xx CoreML smoke check skipped: {result['reason']}")
        return result
    ...
    try:
        ...
        # CoreML expects different input format - skip actual inference
        result['status'] = 'ok'
        result['reason'] = 'Model loaded successfully (full inference skipped)'
...
def export_and_package_model(model: nn.Module,
    ...
    export_log = open(export_log_path, 'w')
    ...
    """
    # =================================================================
    # OPENVINO EXPORT (requires ONNX)
    # =================================================================
    if 'openvino' in export_formats:
        logger_inst.info("\n[7/10] Exporting OpenVINO IR...")
        log_to_file("Exporting OpenVINO IR...")
        
        if onnx_path is None or not onnx_path.exists():
            logger_inst.warning("  xx ONNX file required for OpenVINO export")
            export_results['openvino'] = {
                'path': None, 'status': 'skipped',
                'reason': 'ONNX file not available', 'sha256': ''
            }
            log_to_file("  Export skipped: ONNX file not available")
        else:
            result = export_model_openvino_ir(
                onnx_path, subdirs['openvino'], arch_name, force, logger_inst
            )
            export_results['openvino'] = result
            
            if result['status'] == 'ok':
                xml_path = result['path']
                checksums[str(xml_path.name)] = result['sha256']
                
                smoke = smoke_check_openvino(xml_path, sample_input, logger_inst)
                smoke_results['openvino'] = smoke
                log_to_file(f"  Smoke check: {smoke['status']} - {smoke['reason']}")
            else:
                log_to_file(f"  Export {result['status']}: {result['reason']}")
    
    # =================================================================
    # COREML EXPORT (requires ONNX)
    # =================================================================
    if 'coreml' in export_formats:
        logger_inst.info("\n[8/10] Exporting CoreML...")
        log_to_file("Exporting CoreML...")
        
        if onnx_path is None or not onnx_path.exists():
            logger_inst.warning("  xx ONNX file required for CoreML export")
            export_results['coreml'] = {
                'path': None, 'status': 'skipped',
                'reason': 'ONNX file not available', 'sha256': ''
            }
            log_to_file("  Export skipped: ONNX file not available")
        else:
            coreml_path = subdirs['coreml'] / f"{arch_name}.mlmodel"
            result = export_model_coreml(
                onnx_path, coreml_path, arch_name, force, logger_inst
            )
            export_results['coreml'] = result
            
            if result['status'] == 'ok':
                checksums[str(coreml_path.name)] = result['sha256']
                
                smoke = smoke_check_coreml(coreml_path, sample_input, logger_inst)
                smoke_results['coreml'] = smoke
                log_to_file(f"  Smoke check: {smoke['status']} - {smoke['reason']}")
            else:
                log_to_file(f"  Export {result['status']}: {result['reason']}")
    """
    ...
    log_to_file(f"  Created: metadata.json")

    # 2. class_mapping.json
    if hasattr(model, 'classes') or NUM_CLASSES:
        class_names = getattr(model, 'classes', [f'class_{i}' for i in range(num_classes)])
        ...
        log_to_file(f"  Created: class_mapping.json")
    ...
    log_to_file(f"  Created: export_status.json")
    ...
    if smoke_results:
        ...
        log_to_file(f"  Created: smoke_check_results.json")
...
def test_export_state_dict():
    ...
    try:
        ...
        smoke = smoke_check_state_dict(output_path, 'TestModel', sample_input, logger)
...
class DynamicConv(nn.Module):
    ...
    def forward(self, x):
        ...
        weight_bank_flat = self.weight_bank.view(K, self.out_channels, -1) * self.scale # (K, out, in*k*k)
...
def train_backbone_with_metrics(backbone_name, model, train_ds, val_ds, 
    ...
    if EXPORT_FORMATS and ENABLE_EXPORT:
        ...
        if not ENABLE_EXPORT:
            logger.info(f"Export disabled (ENABLE_EXPORT = False)")
            metrics['export_summary'] = {
                'status': 'disabled',
                'reason': 'Export functionality disabled via ENABLE_EXPORT flag'
            }
        else:
            logger.info(f"Export skipped (no formats configured)")
            metrics['export_summary'] = {
                'status': 'skipped',
...
def prepare_datasets_for_backbone(backbone_name, size=224):
    ...
    logger.info(f"Prepared datasets for {backbone_name}: train={len(train_ds)}, val={len(val_ds)}, classes={NUM_CLASSES}")
...
class CustomCoAtNet(nn.Module):
    ...
    def __init__(self, num_classes=1000):
        ...
        # ULTRA DEEP STEM: 224 -> 56 with residual connections
        self.stem = nn.Sequential(
            # Block 1: 224 -> 112
            ...
            nn.BatchNorm2d(48),
            nn.GELU(),
            nn.Conv2d(48, 64, 3, 1, 1, bias=False),
            nn.BatchNorm2d(64),
            nn.GELU(),
        ...
        # STAGE 1: ULTRA DEEP CNN (56x56, 96 -> 128 -> 192)
        # Mix of different block types for diversity
        stage1_blocks = []
        
        # Downsample: 56 -> 28
        stage1_blocks.append(InvertedResidualBlock(96, 128, 2, 6))
        
        # Dense connections at 28x28
        stage1_blocks.extend([
            InvertedResidualBlock(128, 128, 1, 6),
            InvertedResidualBlock(128, 128, 1, 4),
            InvertedResidualBlock(128, 128, 1, 6),
            InvertedResidualBlock(128, 128, 1, 4),
            InvertedResidualBlock(128, 128, 1, 6),
            InvertedResidualBlock(128, 128, 1, 4),
        ...
        # Channel expansion
        stage1_blocks.append(InvertedResidualBlock(128, 192, 1, 6))
        stage1_blocks.append(InvertedResidualBlock(192, 192, 1, 4))
        
        self.stage1 = nn.Sequential(*stage1_blocks)
      
        # STAGE 2: DEEP CNN + Progressive Attention (28x28, 192 -> 256 -> 384)
        stage2_cnn_blocks = []
        
        # Downsample: 28 -> 14
        stage2_cnn_blocks.append(InvertedResidualBlock(192, 256, 2, 6))
        
        # Very deep at 14x14
        stage2_cnn_blocks.extend([
            InvertedResidualBlock(256, 256, 1, 6),
            InvertedResidualBlock(256, 256, 1, 4),
            InvertedResidualBlock(256, 256, 1, 6),
            InvertedResidualBlock(256, 256, 1, 4),
            InvertedResidualBlock(256, 256, 1, 6),
            InvertedResidualBlock(256, 256, 1, 4),
        ...
        # Channel expansion to 384
        stage2_cnn_blocks.append(InvertedResidualBlock(256, 384, 1, 6))
        stage2_cnn_blocks.append(InvertedResidualBlock(384, 384, 1, 4))
        
        self.stage2_cnn = nn.Sequential(*stage2_cnn_blocks)
        
        # Progressive attention blocks with residual
        self.stage2_attn_blocks = nn.ModuleList([
            nn.ModuleDict({
                'attn': TransformerEncoderBlock(384, num_heads=12, mlp_ratio=4.0),
                'proj': nn.Identity()
            })
            for _ in range(6)  # 6 attention blocks
        ])
        
        # STAGE 3: ULTRA DEEP Transformer (14x14, 384 -> 768 -> 1024)
        self.stage3_proj = nn.Sequential(
            nn.Conv2d(384, 512, 1, 1, 0, bias=False),
            nn.BatchNorm2d(512),
            nn.GELU(),
        ...
        # ULTRA DEEP transformer: 24 blocks with LayerScale
        self.stage3_transformer = nn.ModuleList([
            TransformerEncoderBlockWithLayerScale(
                768, 
                num_heads=16, 
                mlp_ratio=4.0,
                init_values=1e-6  # Very small for ultra deep
            )
            for _ in range(24)  # 24 transformer blocks!
        ])
    ...
    def forward(self, x):
        ...
        # Stage 1: Ultra deep CNN
        x = self.stage1(x)  # (B, 192, 28, 28)
        
        # Stage 2: Deep CNN + Progressive Attention
        x = self.stage2_cnn(x)  # (B, 384, 14, 14)
        
        # Apply attention blocks with strong residuals
        B, C, H, W = x.shape
        x_seq = x.flatten(2).transpose(1, 2)  # (B, 196, 384)
        
        for attn_module in self.stage2_attn_blocks:
            identity = x_seq
            x_seq = attn_module['attn'](x_seq)
            x_seq = x_seq + identity  # Strong residual
        
        x = x_seq.transpose(1, 2).reshape(B, C, H, W)
        
        # Stage 3: Ultra deep transformer
        x = self.stage3_proj(x)  # (B, 768, 14, 14)
        
        B, C, H, W = x.shape
        x = x.flatten(2).transpose(1, 2)  # (B, 196, 768)
        
        # Apply ultra deep transformer with skip connections every 4 blocks
        for i, transformer_block in enumerate(self.stage3_transformer):
            identity = x
            x = transformer_block(x)
            
            # Add skip connection every 4 blocks for better gradient flow
            if (i + 1) % 4 == 0:
                x = x + identity
        
        # Feature enhancement
        x = x + self.feature_enhance(x)
...
class CustomMaxViT(nn.Module):
    ...
    def __init__(self, num_classes=1000):
        ...
        # FIXED: Optimized stem (was too aggressive)
        self.stem = nn.Sequential(
            # 224 -> 112
            CoreImageBlock(3, 64, 3, 2, 1, activation='gelu'),
            CoreImageBlock(64, 64, 3, 1, 1, activation='gelu'),
            ...
            # 112 -> 56
            CoreImageBlock(64, 96, 3, 2, 1, activation='gelu'),
            CoreImageBlock(96, 96, 3, 1, 1, activation='gelu'),
        )
        
        # STAGE 1: CNN Feature Extraction (56x56, 96 -> 192)
        stage1_blocks = []
        
        # Downsample: 56 -> 28
        stage1_blocks.append(InvertedResidualBlock(96, 128, 2, 4))
        
        # Feature extraction at 28x28
        for _ in range(4):  # Reduced from 5 to 4
            stage1_blocks.append(InvertedResidualBlock(128, 128, 1, 4))
        
        # Expand to 192
        stage1_blocks.append(InvertedResidualBlock(128, 192, 1, 4))
        
        self.stage1 = nn.Sequential(*stage1_blocks)
        
        # STAGE 2: CNN + Light Attention (28x28, 192 -> 384)
        stage2_cnn_blocks = []
        
        # Downsample: 28 -> 14
        stage2_cnn_blocks.append(InvertedResidualBlock(192, 256, 2, 4))
        
        # Deep feature extraction at 14x14
        for _ in range(5):  # Reduced from 6
            stage2_cnn_blocks.append(InvertedResidualBlock(256, 256, 1, 4))
        
        # Expand to 384
        stage2_cnn_blocks.append(InvertedResidualBlock(256, 384, 1, 4))
        
        self.stage2_cnn = nn.Sequential(*stage2_cnn_blocks)
        
        # FIXED: Minimal attention (2 blocks instead of 4)
        self.stage2_attn = nn.ModuleList([
            TransformerEncoderBlockWithLayerScale(
                384, 
                num_heads=8, 
                mlp_ratio=3.0,
                init_values=0.1  # Larger for better initial gradients
            )
            for _ in range(2)  # Just 2 attention blocks
        ])
        
        # STAGE 3: Transformer (14x14, 384 -> 768)
        self.stage3_proj = nn.Sequential(
            CoreImageBlock(384, 512, 3, 1, 1, activation='gelu'),
            nn.Dropout2d(0.1),
            CoreImageBlock(512, 768, 3, 1, 1, activation='gelu'),
        )
        
        # CRITICAL FIX: Only 8 transformer blocks (was 12-28!)
        self.stage3_transformer = nn.ModuleList([
            TransformerEncoderBlockWithLayerScale(
                768, 
                num_heads=12, 
                mlp_ratio=4.0,
                init_values=0.1  # Strong initial values
            )
            for _ in range(8)  # OPTIMAL: 8 blocks
        ])
        
        # FIXED: Simplified feature refinement
        self.norm = nn.LayerNorm(768)
        ...
        # CRITICAL: Better initialization
        self.apply(self._init_weights)
        
        # Initialize positional-like parameters with smaller values
        for module in self.modules():
            if isinstance(module, TransformerEncoderBlockWithLayerScale):
                # Ensure LayerScale starts with good values
                if hasattr(module, 'gamma_1'):
                    nn.init.constant_(module.gamma_1, 0.1)
                if hasattr(module, 'gamma_2'):
                    nn.init.constant_(module.gamma_2, 0.1)
    ...
    def forward(self, x):
        ...
        # Stage 2: CNN + Light Attention
        x = self.stage2_cnn(x)  # (B, 384, 14, 14)
        
        # Apply minimal attention
        B, C, H, W = x.shape
        x_seq = x.flatten(2).transpose(1, 2)  # (B, 196, 384)
        
        # FIXED: Simple attention with strong residuals
        for attn_block in self.stage2_attn:
            x_seq = x_seq + attn_block(x_seq)  # Direct residual
        ...
        # FIXED: Optimal transformer depth with strong skip connections
        for i, transformer_block in enumerate(self.stage3_transformer):
            x = x + transformer_block(x)  # Always use direct residual
            ...
            for retry in range(max_retries):
                try:
                    ...
                    if retry == max_retries - 1:
                        ...
                        logger_inst.info(f"  Retrying in 2 seconds...")
                        time.sleep(2)
...
def smart_adapt_pretrained_weights(pretrained_state_dict: dict, 
    ...
    logger_inst.info(f"  Step 1: Direct key matching...")
    ...
    logger_inst.info(f"  Step 2: Shape-based matching...")
    ...
    matched_pretrained = set(adapted_dict.values())
    ...
    logger_inst.info(f"  Step 3: Pattern-based transformations...")
...
def load_pretrained_to_model(model: nn.Module, 
    ...
    try:
        ...
        if matched_count > 0:
            ...
            logger_inst.warning(f"  No compatible parameters found")
            logger_inst.info(f"  Training from scratch")
...
def verify_swin_transformer(model, backbone_name, input_size=224, num_classes=None, device=DEVICE):
...
def verify_dynamic_conv(model, backbone_name, input_size=224, num_classes=None, device=DEVICE):
...
def verify_mobileone_reparam(model, backbone_name, input_size=224, num_classes=None, device=DEVICE):
...
def verify_model_architecture(model, backbone_name, input_size=224, num_classes=None, device=DEVICE):
    ...
    if 'Swin' in backbone_name:
        if verify_swin_transformer(model, backbone_name, input_size, num_classes, device):
            checks_passed += 1
    else:
        checks_passed += 1
    
    if 'Dynamic' in backbone_name:
        if verify_dynamic_conv(model, backbone_name, input_size, num_classes, device):
            checks_passed += 1
    else:
        checks_passed += 1
    
    if 'MobileOne' in backbone_name:
        if verify_mobileone_reparam(model, backbone_name, input_size, num_classes, device):
            checks_passed += 1
    else:
        checks_passed += 1
...
def create_custom_backbone(name: str, num_classes: int):
    ...
    if name_lower in backbone_map:
        ...
        logger.info(f"Creating {name} with {num_classes} classes...")
        model = backbone_map[name_lower](num_classes=num_classes)
        ...
        if not verify_model_architecture(model, name, IMG_SIZE, num_classes, DEVICE):
...
def create_custom_backbone_safe(name: str, num_classes: int):
...
def run_optimized_pipeline():
    ...
    if not RAW_DIR.exists():
        ...
        logger.info(f"  2. Contains subdirectories for each disease class")
        logger.info(f"  3. Each subdirectory contains image files (.jpg, .png, etc.)")
    ...
    logger.info(f"Dataset loaded: {len(full_dataset)} samples, {NUM_CLASSES} disease classes")
    ...
    if successful_runs:
        if ENABLE_KFOLD_CV:
            avg_kfold_acc = np.mean([r['kfold_mean_accuracy'] for r in successful_runs 
                                    if r.get('kfold_mean_accuracy') is not None])
        avg_final_acc = np.mean([r['final_accuracy'] for r in successful_runs])
        ...
        avg_test_acc = np.mean(test_accs) if test_accs else None
    ...
    with open(report_path, 'w') as f:
        ...
        if successful_runs:
            ...
            if ENABLE_KFOLD_CV:
                f.write(f"  Average K-fold CV accuracy: {avg_kfold_acc:.4f}")
...
def debug_model_creation(backbone_name, debug_log):
    ...
    try:
        ...
        model = create_custom_backbone_safe(backbone_name, NUM_CLASSES)
        debug_log.check("Model Creation", "PASS", f"Model created successfully")
...
def debug_forward_pass(model, backbone_name, debug_log):
...
def debug_single_epoch(model, backbone_name, debug_log):
    ...
    try:
        ...
        train_loader = create_optimized_dataloader(train_ds, DEBUG_BATCH_SIZE, shuffle=True)
        val_loader = create_optimized_dataloader(val_ds, DEBUG_BATCH_SIZE, shuffle=False)
...
def debug_full_training(backbone_name, debug_log):
    ...
    debug_log.log(f"Using {DEBUG_EPOCHS_HEAD} head epochs, {DEBUG_EPOCHS_FINETUNE} finetune epochs")
    debug_log.log(f"{'='*60}")
    ...
    try:
        ...
        final_model, best_acc, history, metrics = train_backbone_with_metrics(
            backbone_name, model, train_ds, val_ds,
            epochs_head=DEBUG_EPOCHS_HEAD,
            epochs_finetune=DEBUG_EPOCHS_FINETUNE
        )
...
def run_debug_mode():
    ...
    debug_log.log(f"  Batch Size: {DEBUG_BATCH_SIZE}")
    debug_log.log(f"  Device: {DEVICE}")
    debug_log.log(f"  Num Classes: {NUM_CLASSES if NUM_CLASSES else 'Auto-detect'}")
    ...
    try:
        ...
        if DEBUG_FUNCTION != 'model_creation' and DEBUG_FUNCTION != 'smoke_tests':
            ...
            if NUM_CLASSES is None:
                ...
                debug_log.log(f"  Auto-detected {NUM_CLASSES} classes")
        ...
        if DEBUG_FUNCTION == 'model_creation':
            ...
            model = debug_model_creation(DEBUG_BACKBONE, debug_log)
            verify_model_architecture(model, DEBUG_BACKBONE, IMG_SIZE, NUM_CLASSES, DEVICE)
...
if __name__ == "__main__":
    ...
    if DEBUG_MODE:
        ...
        logger.info(f"  DBT_DEBUG_HEAD_EPOCHS={DEBUG_EPOCHS_HEAD}")
        logger.info(f"  DBT_DEBUG_FT_EPOCHS={DEBUG_EPOCHS_FINETUNE}")
        logger.info(f"  DBT_DEBUG_BATCH_SIZE={DEBUG_BATCH_SIZE}")
        logger.info(f"\nAvailable Debug Functions:")
        for func, desc in DEBUG_FUNCTIONS.items():
