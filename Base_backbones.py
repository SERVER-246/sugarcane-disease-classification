...
def run_optimized_pipeline():
    ...
    try:
        ...
        viz_index = {
            ...
            'comparison_plots': {
                'kfold_comparison': str(kfold_plot_path) if ENABLE_KFOLD_CV and kfold_results_dict else None,
    ...
    logger.info(f"  Comparison plots: 2")
    ...
    if viz_index_file.exists():
        ...
        logger.warning(f"  x Visualization index not found")
...
def plot_per_class_metrics(y_true, y_pred, class_names, save_path, title="Per-Class Metrics"):
    ...
    try:
        ...
        for i, v in enumerate(support):
            ax2.text(i, v + max(support)*0.02, str(int(v)), 
        ...
        metrics_dict = {
            ...
            'weighted_avg': {
                'precision': float((precision * support).sum() / support.sum()),
                'recall': float((recall * support).sum() / support.sum()),
                'f1_score': float((f1 * support).sum() / support.sum())
            }
        }
...
def plot_kfold_results(kfold_results, save_path, title="K-Fold Cross-Validation Results"):
    ...
    try:
        ...
        for i, (bar, mean, std) in enumerate(zip(bars, mean_accs, std_accs)):
